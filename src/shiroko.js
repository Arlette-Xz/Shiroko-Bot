...
export async function handler(chatUpdate) {
    ...
    try {
        ...
        // Optimización: Acceso directo a DB para evitar búsquedas repetitivas
        const users = global.db.data.users
        const chats = global.db.data.chats
        const settingsDB = global.db.data.settings

        if (!users[sender]) users[sender] = { name: mName, exp: 0, coin: 0, bank: 0, level: 0, health: 100, genre: "", birth: "", marry: "", description: "", packstickers: null, premium: false, premiumTime: 0, banned: false, bannedReason: "", commands: 0, afk: -1, afkReason: "", warn: 0 }
        if (!chats[mChat]) chats[mChat] = { isBanned: false, isMute: false, welcome: global.modes.welcome, sWelcome: "", sBye: "", detect: global.modes.detect, primaryBot: null, modoadmin: global.modes.modoadmin, antiLink: global.modes.antilink, nsfw: global.modes.nsfw, economy: global.modes.economy, gacha: global.modes.gacha }
        if (!settingsDB[this.user.jid]) settingsDB[this.user.jid] = { self: global.modes.self, jadibotmd: global.modes.jadibotmd, autoread: global.modes.autoread, autoreaction: global.modes.autoreaction, anticall: global.modes.anticall }

        const user = users[sender]
        const chat = chats[mChat]
        ...
        if (m.isGroup) {
            ...
            if (cached && (now - cached.timestamp) < 15000) { // Caché más agresiva (15s)
        ...
        for (let i = 0; i < pluginNames.length; i++) {
            ...
            // Ejecución de "all" sin bloqueo innecesario
            ...
            if (match && match[0] && (usedPrefix = match[0][0])) {
                ...
                // Lógica de seguridad y PrimaryBot intacta ꕤ
                ...
                break // ¡Súper importante! Detiene la búsqueda una vez ejecutado el comando ✰
            }
        }
    } catch (err) {
        console.error(err)
